...
int resretButton = 0; // pin number with RESET button when pressed clears stored information and activates AP mode
...
void setup() {
  ...
  // uncomment next line for first flash, then comment it again and reflash (EEPROM must be written before reading from it)
...
// Sets default values and sets board to boot in AP mode
void clearEEPROM()
{
  isActivated = false;
  EEPROM.put(0, isActivated);
